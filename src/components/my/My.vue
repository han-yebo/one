<template>
  <div class="content">
    <header>
      <div class="box">
        <div class="xs">
          <img :src="list.avatar" alt/>
          <p class="name"  @click="go">{{list.nickname}}</p>
          <p class="yk">去约课</p>
        </div>
        <div class="xs1">
          <ul>
            <li @click="$router.push('/mycourse')">
              <p class="p1">0</p>
              <p class="p2">我的特色课</p>
              <p class="p3">已购特色课程的学习</p>
            </li>
            <li @click="$router.push('/home/record')">
              <p class="p1">0</p>
              <p class="p2">一对一辅导</p>
              <p class="p3">我的一对一考试辅导</p>
            </li>
            <li @click="$router.push('/balance')">
              <p class="p1">0.00</p>
              <p class="p2">剩余学习币</p>
              <p class="p3">查看剩余学习币</p>
            </li>
          </ul>
        </div>
      </div>
    </header>
    <div class="content1">
      <div class="xs3">
        <img
          src="data:image/png;base64,"
          alt
        />
        <div class="xs4">
          <p class="p5">邀请好友注册APP，享多重好礼</p>
          <p class="p5">限时特惠，多邀多得</p>
        </div>
        <van-icon name="arrow" />
      </div>
      <div class="xs5">
        <p class="xg">课程相关</p>
        <ul class="u1">
          <li @click="$router.push('/follow')">
            <img
              src="data:image/png;base64,"
              alt
            />
            <p class="p6">关注的老师</p>
          </li>
          <li @click="$router.push('/collection')">
            <img
              src="data:image/png;base64,"
              alt
            />
            <p class="p6">我的收藏</p>
          </li>
        </ul>
        <p class="xg">订单相关</p>
        <ul class="u1">
          <li>
            <img
              src="data:image/png;base64,"
              alt
            />
            <p class="p6">课程订单</p>
          </li>
          <li>
            <img
              src="data:image/png;base64,"
              alt
            />
            <p class="p6">会员订单</p>
          </li>
          <li>
            <img
              src="data:image/png;base64,"
              alt
            />
            <p class="p6">约课订单</p>
          </li>
        </ul>
        <p class="xg">我的账户</p>
        <ul class="u1">
          <li>
            <img
              src="data:image/png;base64,"
              alt
            />
            <p class="p6">优惠券</p>
          </li>
          <li>
            <img
              src="data:image/png;base64,"
              alt
            />
            <p class="p6">学习卡</p>
          </li>
          <li>
            <img
              src="data:image/png;base64,"
              alt
            />
            <p class="p6">会员</p>
          </li>
        </ul>
        <p class="xg">自助服务</p>
        <ul class="u1">
          <li>
            <img
              src="data:image/png;base64,"
              alt
            />
            <p class="p6">我的消息</p>
          </li>
          <li>
            <img
              src="data:image/png;base64,"
              alt
            />
            <p class="p6">意见反馈</p>
          </li>
          <li>
            <img
              src="data:image/png;base64,"
              alt
            />
            <p class="p6">在线客服</p>
          </li>
           <li>
            <img
              src="data:image/png;base64,"
              alt
            />
            <p class="p6">设置</p>
          </li>
        </ul>
      </div>
    </div>
  </div>
</template>
<script>
export default {
  beforeRouteEnter(to, from, next) {
    if (localStorage.getItem("token")) {
      next();
    } else {
      next("/login");
    }
  },
  data() {
    return {
      list: []
    };
  },
  mounted() {
    this.get();
  },
  methods: {
    get() {
      this.$http.user().then(res => {
        console.log(res);
        this.list = res.data.data;
      });
    },
    go(){
      this.$router.push('/mydetail')
     
    }
  }
};
</script>
<style scoped lang="scss">
header {
  background: rgb(250, 108, 26);
  height: 2rem;
  padding: 0 0.35rem;
  .box {
    background: #fff;
    // height: 3rem;
    box-shadow: 0 0 0.1rem #ccc;
    transform: translateY(0.8rem);
    border-radius: 0.1rem;
    // padding-bottom: 0.3rem;
    .xs {
      display: flex;
      height: 1.4rem;
      padding: 0.3rem 0 0 0.3rem;
      justify-content: space-between;
      align-items: center;
    }
    img {
      width: 1.2rem;
      height: 1.2rem;
    }
    .name {
      font-size: 0.3rem;
      flex: 1;
      margin-left: 0.2rem;
    }
    .yk {
      font-size: 0.25rem;
      background: orangered;
      color: #fff;
      width: 1.1rem;
      height: 0.5rem;
      text-align: center;
      line-height: 0.5rem;
      border-top-left-radius: 0.3rem;
      border-bottom-left-radius: 0.3rem;
    }
  }
}
.xs1 {
  ul {
    display: flex;
    padding: 0.3rem 0.1rem;
    li {
      text-align: center;
      flex: 1;
    }
    .p1 {
      font-size: 0.55rem;
      color: rgb(250, 108, 26);
    }
    .p2 {
      font-size: 0.28rem;
    }
    .p3 {
      font-size: 0.23rem;
      color: #ccc;
    }
  }
}
.content1 {
  padding: 0 0.25rem 2rem 0.25rem;
  margin-top: 2.4rem;
}
.xs3 {
  display: flex;
  padding: 0.15rem 0.3rem;
  background: linear-gradient(-90deg, #f2995a, #eb6100);
  align-items: center;
  border-radius: 0.5rem;
  justify-content: space-between;
  img {
    width: 0.4rem;
    height: 0.36rem;
  }
}
.xs4 {
  flex: 1;
  margin-left: 0.2rem;
}
.p5 {
  font-size: 0.2rem;
  color: #fff;
}
.van-icon {
  font-size: 0.5rem;
  color: #fff;
}
.xg {
  font-size: 0.32rem;
  margin-top: 0.3rem;
}
.u1 {
  display: flex;
  font-size: 0;
  margin-top: 0.3rem;
  border-bottom: 0.01rem solid #eee;

  li {
    text-align: center;
    // line-height: 1.2rem;
    width: 1.72rem;
    height: 1.2rem;
    img {
      width: 0.42rem;
      height: 0.45rem;
    }
    .p6 {
      font-size: 0.22rem;
    }
  }
}
</style>